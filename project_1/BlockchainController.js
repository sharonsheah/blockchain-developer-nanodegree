...
class BlockchainController {
    ...
    validateChain() {
        this.app.post("/blocks/validateChain", async (req, res) => {
            if(req.params.address) {
                try {
                    let validateChain = await this.blockchain.validateChain();
                    if(validateChain){
                        return res.status(200).json(validateChain);
                    } else {
                        return res.status(404).send("Chain not valid!");
                    }
                } catch (error) {
                    return res.status(500).send("An error happened!");
                }
            } else {
                return res.status(500).send("Chain Not Found! Review the Parameters!");
            }
